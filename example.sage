...
def find_roots_groebner(pr,desired_solution, unknown_modular, polynomials,bounds):
    """
    ...
    for ii in range(len(s)):
        ...
        logging.debug(f'Test for Sequence: s(x0, y0, z0) mod modular = {s[ii](desired_solution)% unknown_modular}')
        logging.debug(f'Test for Sequence: s(x0, y0, z0) = {s[ii](desired_solution)} over Z')
    while len(s) > 0:
...
def modular_gifp(desired_solution, unknown_modular, f, M, N1, N2, m, t, X, Y, Z):
    ...
    for ii in range(m + 1):
        for jj in range(m - ii + 1):
            ...
            logging.debug(f'Test for shift: g(x0, y0, z0) mod modular = {g(desired_solution)% unknown_modular}')
    ...
    for poly in polynomials:
        ...
        logging.debug(f'Test for reconstruct_polynomial: g(x0, y0, z0) mod modular = {poly(desired_solution)% unknown_modular}')
        logging.debug(f'Test for reconstruct_polynomial: g(x0, y0, z0) = {poly(desired_solution)} over Z')
    for roots in find_roots_groebner(pr,desired_solution, unknown_modular, polynomials,[X, Y, Z]):
...
def attack_gifp_instance(modulus_bit_length, alpha, gamma, beta1, beta2, m=1, seed=None, t=None):
    ...
    gb_tmp = []
    for x0, y0, z0 in modular_gifp(desired_solution, unknown_modular, f, M, N1, N2, m, t, X, Y, Z):
        v = int(f(x0, y0, z0))
        if v == 0:
            p2 = N2/z0
            q2 = z0
...
seed = 1707216009778938
modulus_bit_length, alpha, gamma, beta1, beta2, m = 200, 0.1, 0.7, 0.1, 0.15, 4
